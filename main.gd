...
func show_todos():
	...
	var view = preload("res://todos/todos.tscn").instance()
	view.bind_viewModel(viewMode)
	add_child(view)

func show_calculator():
	var ViewModel = preload("res://calculator/calculator.gd")
	var Model = ViewModel.Model
	var model = Model.new()
	var viewModel = MVC.create_viewModel(model, ViewModel)
	var view = preload("res://calculator/calculator_view.tscn").instance()
